...
function buildStars(rating) {
	...
	for (let i = 1; i <= 5; i++) {
		if (rating >= i) {
			...
			html += '<span aria-hidden="true" class="icon-star icon-star-half">⭐</span>';
		} else {
...
function buildCard(recipe) {
	const desktopTagsHTML = recipe.tags
		.map(tag => `<span class="tag tag-desktop">${tag}</span>`)
		.join('');

	const mobileTagsHTML = recipe.tags
		.map(tag => `<span class="tag">${tag}</span>`)
		.join('');

	const starsHTML = buildStars(recipe.rating);
	const ratingLabel = `Rating: ${recipe.rating} out of 5 stars`;

	return `
		<article class="recipe-card">
			<div class="recipe-image">
				<img src="${recipe.image}" alt="${recipe.name}">
				<div class="tag-group mobile-tags">${mobileTagsHTML}</div>
			</div>
			<div class="recipe-info">
				<div class="tag-group desktop-tags">${desktopTagsHTML}</div>
				<h2>${recipe.name}</h2>
				<span class="rating" role="img" aria-label="${ratingLabel}">
					${starsHTML}
				</span>
...
function renderRecipes(list) {
	...
	if (list.length === 0) {
		main.innerHTML = '<p class="no-results">No recipes found.</p>';
	} else {
		main.innerHTML = list.map(buildCard).join('');
	}
}
